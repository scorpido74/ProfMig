...
function Assert-ProfMigMigrationAllowed {
    ...
    .DESCRIPTION
        Throws a typed ProfMig exception when the validation summary contains
        one or more critical failures.

        Insufficient destination storage is classified separately from other
        validation failures so callers can return a predictable process exit
        code.
    ...
    if ($null -eq $ValidationSummary) {
        ...
        throw (
            New-ProfMigException `
                -Message 'Migration cannot start because no validation summary was provided.' `
                -Category 'ValidationError' `
                -Severity 'Critical' `
                -RecoveryAction 'Stop' `
                -Reason 'ValidationSummaryMissing'
        )
    }
    ...
    if (
        ...
        throw (
            New-ProfMigException `
                -Message 'Migration cannot start because the validation summary is invalid.' `
                -Category 'ValidationError' `
                -Severity 'Critical' `
                -RecoveryAction 'Stop' `
                -Reason 'ValidationSummaryInvalid'
        )
    }
        ...
        if ($insufficientStorage) {
            ...
            throw (
                New-ProfMigException `
                    ...
                    -Severity 'Critical' `
                    -RecoveryAction 'Stop' `
                    -Reason 'InsufficientDestinationStorage'
            )
        }
